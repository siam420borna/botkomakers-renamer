...
async def start(client, message):
    if is_banned(message.from_user.id):
        return await message.reply('⛔ আপনি ব্যান হয়েছেন।')
    add_user(message.from_user.id)
    user = message.from_user

    try:
        log_msg = f"🆕 নতুন ইউজার!

👤 নাম: [{user.first_name}](tg://user?id={user.id})
🆔 ID: `{user.id}`
🌐 ইউজারনেম: @{user.username if user.username else "N/A"}
🌍 ভাষা: `{user.language_code}`"
        await client.send_message(-1002589776901, log_msg)
